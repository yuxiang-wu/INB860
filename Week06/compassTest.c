...
//  while(nMotorRunState[Right] != runStateIdle){
  while(compass() - cur_compass < 360){
    ...
    nxtDisplayStringAt(0,31,"%f", compass());
  }
  motor[Left]=0;
  motor[Right]=0;
  return false;
...
task main(){
  ...
  motor[Left]=0;
  motor[Right]=0;
  nxtDisplayStringAt(0,31, "%d %d", leftMotorEncode, rightMotorEncode);
  nxtDisplayStringAt(1,39, "%f", compass());
