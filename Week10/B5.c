...
void recordAdj(int from, int to){
  nxtDisplayTextLine(5, "%d %d", from, to);

  adjMat[from][adjSize[from]] = to;
  adjSize[from]++;
  adjMat[to][adjSize[to]] = from;
  adjSize[to]++;
}
  ...
  if(!bt){ // Not backtracking
      ...
      if(wasUTurn){
        ...
        if(c == 0){ // New leaf
          ...
          if(size > 0)
            recordAdj(logbook[size-1][2], nodeCounter);
          else
            recordAdj(0, nodeCounter);
          ...
          nxtDisplayTextLine(4, "New node %d", nodeCounter);
          if(size > 1)
            recordAdj(logbook[size-2][2], nodeCounter);
          else
            recordAdj(0, nodeCounter);
        }
        nodeCounter++;
...
void writeAdjMatFile(){
    const string filename ="IncidenceMatrix.dat";
    TFileIOResult nIoResult;
    ...
    for(int i = 0; i <= nodeCounter; i++){
        StringFormat(str, "%d ", i);
        WriteString(filehandle, nIoResult, str);
        for(int j = 0; j < adjSize[i]; j++){
            StringFormat(str, "%d ", adjMat[i][j]);
            WriteString(filehandle, nIoResult, str);
            nxtScrollText("%d ",adjMat[i][j]);
        }
...
task main(){
  ...
  nxtDisplayTextLine(6, "#nodes = %d", nodeCounter + 1);
